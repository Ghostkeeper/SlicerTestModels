...
//Create a pillar in a certain grid cell.
module pillar(x, y) {
	translate([x * block_width + block_width / 2, y * block_width + block_width / 2, 0]) {
		cylinder(r=block_width / 4, h=wall_height / 2);
	}
}

//Create random pillars in the maze.
module pillars() {
	has_pillar = rands(0, 1, columns * rows);
	for(x = [0 : columns - 1]) {
		for(y = [0 : rows - 1]) {
			if(has_pillar[x * columns + y] > 1 - pillar_chance) {
				pillar(x, y);
			}
		}
	}
